...
Stores results in GTM.PUBLIC.COMPANY_WEB_SIGNALS_ACCOUNT_SCORE_40 table in Snowflake.
...
class CompanyWebSignals:
    ...
    def create_table_if_not_exists(self):
        """Create the COMPANY_WEB_SIGNALS_ACCOUNT_SCORE_40 table if it doesn't exist."""
        create_sql = """
        CREATE TABLE IF NOT EXISTS GTM.PUBLIC.COMPANY_WEB_SIGNALS_ACCOUNT_SCORE_40 (
        ...
        try:
            ...
            conn.close()
            print(f"✓ Table GTM.PUBLIC.COMPANY_WEB_SIGNALS_ACCOUNT_SCORE_40 ready")
            return True
        ...
        for attempt in range(4):
            try:
                ...
                if e.response is not None and e.response.status_code == 429 and attempt < 3:
                    ...
                    print(f"  ⚠ Exa rate limited; retrying in {delay}s")
                    time.sleep(delay)
                    continue
    ...
    def _extract_tools(self, text: str) -> Dict[str, List[str]]:
        ...
        def contains_tool(tool: str) -> bool:
            return bool(re.search(rf'(?<!\w){re.escape(tool.lower())}(?!\w)', text_lower))

        orchestration = [t for t in self.ORCHESTRATION_TOOLS if contains_tool(t)]
        data_tools = [t for t in self.DATA_TOOLS if contains_tool(t)]
    ...
    def upsert_to_snowflake(self, data: Dict) -> bool:
        ...
        merge_sql = """
        MERGE INTO GTM.PUBLIC.COMPANY_WEB_SIGNALS_ACCOUNT_SCORE_40 AS target
        USING (
            SELECT
                %s AS COMPANY_NAME,
                %s AS DOMAIN,
