...
Fetches company overview, jobs, acquisitions, product releases, C-suite hires,
strategic announcements, and SEC filings using Exa API.
...
class CompanyWebSignals:
    ...
    def research_acquisitions(self, company_name: str, domain: str) -> List[Dict]:
        """Research acquisitions."""
        print(f"  → Researching acquisitions...")

        two_years_ago = (datetime.now() - timedelta(days=730)).strftime("%Y-%m-%d")
        results = self._exa_search(
            f"{company_name} acquisition OR acquired",
            num_results=15,
    ...
    def research_sec_filings(self, company_name: str, domain: str) -> List[Dict]:
        """Research SEC filings."""
        print(f"  → Researching SEC filings...")

        two_years_ago = (datetime.now() - timedelta(days=730)).strftime("%Y-%m-%d")
        results = self._exa_search(
            f"{company_name} 10-K filing SEC",
            num_results=10,
            start_published_date=two_years_ago
        )
    ...
    def research_company(self, company_name: str, domain: str) -> Optional[Dict]:
        ...
        try:
            ...
            acquisitions = self.research_acquisitions(company_name, domain)
            ...
            filings = self.research_sec_filings(company_name, domain)

            print(f"\n  ✓ Research complete:")
            print(f"    - Overview: {len(overview)} chars")
            print(f"    - Jobs: {len(jobs)} postings")
            print(f"    - Acquisitions: {len(acquisitions)} found")
    ...
    def upsert_to_snowflake(self, data: Dict) -> bool:
        ...
        USING (
            SELECT
                ...
                TO_VARIANT(PARSE_JSON(%s)) AS ACQUISITIONS,
                ...
                TO_VARIANT(PARSE_JSON(%s)) AS SEC_FILINGS
        ) AS source
        ...
        WHEN MATCHED THEN
            UPDATE SET
                ...
                target.SEC_FILINGS = source.SEC_FILINGS
        WHEN NOT MATCHED THEN
            INSERT (
                COMPANY_NAME, DOMAIN, RESEARCH_DATE, COMPANY_OVERVIEW,
                JOBS, ACQUISITIONS, PRODUCT_RELEASES, C_SUITE_HIRES,
                STRATEGIC_ANNOUNCEMENTS, SEC_FILINGS
            )
            VALUES (
                source.COMPANY_NAME, source.DOMAIN, source.RESEARCH_DATE, source.COMPANY_OVERVIEW,
                source.JOBS, source.ACQUISITIONS, source.PRODUCT_RELEASES, source.C_SUITE_HIRES,
                source.STRATEGIC_ANNOUNCEMENTS, source.SEC_FILINGS
            )
        """
